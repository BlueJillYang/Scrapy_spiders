...
class people_news(scrapy.Spider):
    ...
    def __init__(self, start_url=None):
        ...
        self.test_list = list()
        jobs = set(self.start_urls)
        print '总共： ', len(jobs)
        self.total_amount = len(jobs)
    ...
    def get_parse_link_urls(self, response, url):  # 排序有讲究，有改良方案，后续再说吧
        urls_list = [
            ...
            '//div[contains(@class, "d2tu_content")]/div[@class="d2nav d2nav2"]/following-sibling::ul/li/a/@href',  # 湖北娱乐天下
            '//div[@class="p2j_piclist w980 clear clearfix"]/ul/li/a/@href',  # 文艺星青年
            '//div[@class="box_rt"]/ul[contains(@class, "list_16")]/li/a/@href',  # 重庆原创频道
        ...
        for url_xpath in urls_list:
            ...
            if len(urls) > 0:
                ...
                content_num = '当前处理了%d条, 共%d条'%(self.current_amount, self.total_amount) + '\n'
                self.log.write(content_num)
                return url_xpath
    ...
    def parse_err_detail(self, failure):
        ...
        print response.url, " 页面异常 %s"%response.status
        content = response.url + '  这个频道页面异常!!!\n'
        self.log.write(content)
        ...
        try:
            if response.meta["parse_for_content"]:
                ...
                pubtime2 = response.xpath("//div[@class='box01']//div[@class='fl']") or \
                           ...
                           response.xpath('//td[@id="p_publishtime"]') or \
                           response.xpath('//div[@class="text_c"]/p[@class="sou"]') or \
                           response.xpath(u'//h1/following-sibling::div[contains(text(), "时间")]')
                ...
                if len(pubtime2) > 0 and len(pubtime1) == 0:
                    pubtime2 = pubtime2[0].xpath('string(.)').extract()[0].strip().replace("年", "-").replace("月",
                                                                                                             "-").replace(
                        "日", " ")
                    middle_pubtime1 = re.search(r"(\d{4}-\d{1,2}-\d{1,2}\s\d{1,2}.\d{1,2})", pubtime2)
                    middle_pubtime2 = re.search(r"(\d{4}-\d{1,2}-\d{1,2})", pubtime2)
                    if middle_pubtime1:
                        item["pubtime"] = re.search(r"(\d{4}-\d{1,2}-\d{1,2}\s\d{1,2}:\d{1,2})",
                                                    pubtime2.replace(".", "-")).group(0)
                    elif middle_pubtime2:
                        item["pubtime"] = re.search(r"(\d{4}-\d{1,2}-\d{1,2})", pubtime2.replace(".", "-")).group(0)
                    else:
                ...
                if len(pubtime2) == 0 and len(pubtime1) == 0:
                    ...
                    print response.url, " 这个详细页无发布时间！！！%s"%response.status
                    content = response.url + '  这个详细页无发布时间！！！\n'
                    self.log.write(content)
                    if response.headers.has_key('Last-Modified'):
                        item['pubtime'] = datetime.datetime.strptime(response.headers["Last-Modified"],
                                                                     '%a, %d %b %Y %H:%M:%S GMT').strftime(
                ...
                i = response.url
                if "//world.people.com" in i or "//japan.people.com" in i or '//uk.people.com' in i or \
                        '//usa.people.com' in i or '//australia.people.com' in i or \
                        '//korea.people.com' in i or '//ru.people.com' in response.url:
                    item['area'] = u'国际'
                else:
                    item['area'] = u'全国'
                ...
                title1 = response.xpath("//h1[@id='p_title']") or \
                         ...
                         response.xpath('//td[@id="p_title"]') or \
                         response.xpath('//div[@class="text_c"]/h1')
                item["title"] = ""
                if len(title1) > 0:
                    item['title'] = title1[0].xpath('string(.)').extract()[0].strip()
                if item["title"] == "":
                    self.print_info(response.url + "  这个详细页无标题！！！", "warn", False)
                    print response.url, " 这个详细页无标题！！！%s"%response.status
                    content = response.url + '  这个详细页无标题！！！\n'
                    self.log.write(content)
                ...
                content = response.xpath("//div[@class='box_con']").extract() or \
                          ...
                          response.xpath('//div[@class="gray box_text"]').extract() or \
                          response.xpath('//div[@id="rwb_zw"]')
                if len(content) > 0:
                ...
                if len(medianame1) > 0:
                    ...
                    if len(re.findall(ur"(来源|出处|来自|稿源|出自|来源于)", medianame1)) > 0:
                        if re.search(u'.*?(来源|出处|来自|稿源|出自|来源于)\s*[：:]\s*([a-z|A-Z]+\s[a-z|A-Z]+)', medianame1):
                            item['medianame'] =re.search(u'.*?(来源|出处|来自|稿源|出自|来源于)\s*[：:]\s*([a-z|A-Z]+\s[a-z|A-Z]+)', medianame1).group(2)
                        else:
                ...
                if item['medianame']:
                    if re.search(r'[a-z|A-Z]+\s[a-z|A-Z]+', item['medianame']):
                        item['medianame'] = re.search(r'[a-z|A-Z]+\s[a-z|A-Z]+', item['medianame']).group()
                    else:
                        ...
                        if re.search(r'20\d{2}', item['medianame']):
                ...
                if item["medianame"] == "" or (u"年" in item["medianame"] and u"月" in item["medianame"]):
                    ...
                    if len(test_result) > 0:
                        ...
                        print response.url, " 这个详细页无来源！！！%s"%response.status
                        content = response.url + '  这个详细页无来源！！！\n'
                        self.log.write(content)
                        item["medianame"] = u"人民网"
